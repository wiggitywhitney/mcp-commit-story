...
class TestAIConfigSection:
    ...
    def test_config_init_with_valid_ai_section(self):
        ...
        with patch('src.mcp_commit_story.config.resolve_env_vars', return_value=config_data):
            ...
            assert config._ai['openai_api_key'] == 'sk-test123456'
    ...
    def test_config_init_with_missing_api_key_raises_error(self):
        """Test that missing API key in AI section raises ConfigError."""
        config_data = {
            'ai': {}
        }
        with patch('src.mcp_commit_story.config.resolve_env_vars', return_value=config_data):
            with pytest.raises(ConfigError, match="'ai.openai_api_key' is required"):
                Config(config_data)

    def test_config_init_with_empty_api_key_raises_error(self):
        ...
        with patch('src.mcp_commit_story.config.resolve_env_vars', return_value=config_data):
            with pytest.raises(ConfigError, match="'ai.openai_api_key' cannot be empty"):
                Config(config_data)
...
class TestAIConfigValidationTelemetry:
    ...
    def test_ai_validation_success_telemetry(self, mock_get_metrics):
        ...
        mock_metrics.counter.assert_called_with('config.ai_validation_total', labels={'status': 'success'})
    ...
    def test_ai_validation_failure_telemetry(self, mock_get_metrics):
        ...
        config_data = {
            'ai': {
                'openai_api_key': ''  # Empty key should fail validation
            }
        }
        with patch('src.mcp_commit_story.config.resolve_env_vars', return_value=config_data):
        ...
        mock_metrics.counter.assert_called_with('config.ai_validation_total', labels={'status': 'failure'})
        mock_counter.inc.assert_called_once()
